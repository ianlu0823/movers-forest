...
### 方案 A：GitHub Pages（GitHub Actions 自動部署）

本專案已內建 `.github/workflows/deploy.yml`：**每次 `git push` 到 `main` 分支就會自動在雲端建置並上線**，不需要手動跑部署指令。
...
到 repo 的 **Settings → Pages → Build and deployment → Source**，選 **GitHub Actions**（若用 `gh` CLI：`gh api -X PUT repos/<帳號>/movers-forest/pages -f build_type=workflow`）。

之後改完程式碼，本地驗好，`git push origin main` 即自動部署。
網址會是 `https://<帳號>.github.io/movers-forest/`（`vite.config.js` 已設 `base: "./"`，子路徑可直接運作）。
部署狀態看 repo 的 **Actions** 分頁；失敗會顯示紅色，記得留意。

> 目前已部署於 <https://ianlu0823.github.io/movers-forest/>
